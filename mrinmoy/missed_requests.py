...
if __name__ == "__main__":
    # Load missed index links
    with open("mrinmoy\missed_index.json", "r", encoding="utf-8") as f:
    ...
    # Save all results for missed index diseases
    with open("missed_mayoclinic_disease_data.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)
    # Save diagnosis links for missed index diseases
    with open("missed_mayoclinic_diagnosis_links.json", "w", encoding="utf-8") as f:
        json.dump(diag_links, f, indent=2, ensure_ascii=False)
    # Save failed records for missed index diseases
    with open("missed_mayoclinic_failed_records.json", "w", encoding="utf-8") as f:
        json.dump(failed_records, f, indent=2, ensure_ascii=False)
    print(f"Saved {len(all_results)} missed disease entries, {len(diag_links)} diagnosis links, and {len(failed_records)} failed records.")